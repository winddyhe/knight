        ...
        public override void CSGenerateProcess(CSGenerate rGenerate)
        {
            ...
            foreach (var rType in HotfixSerializerBinaryTypes.Types)
            {
                ...
                foreach(var rMemberInfo in rSerializeMemberInfo)
                {
                    ...
                    if (rMemberInfo.IsDefined(typeof(HotfixSBDynamicAttribute), true) && !IsBaseType(GetMemberType(rMemberInfo), false))
                ...
                foreach(var rMemberInfo in rSerializeMemberInfo)
                {
                    if (rMemberInfo.IsDefined(typeof(HotfixSBDynamicAttribute), false) && !IsBaseType(GetMemberType(rMemberInfo), false))
                    {

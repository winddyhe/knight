...
namespace Knight.Framework.Timeline
{
    ...
    [Serializable]
    [NotKeyable]
    public class UniversalEffectPlayableAsset : PlayableAsset, IPropertyPreview, ITimelineClipAsset
    {
        ...
        #region 扩展参数 与内部强关联的在这里扩展,其他的使用继承扩展
        /// <summary>
        /// 指定播放特效的层级
        /// </summary>
        ...
        /// Transform同步数据
        /// </summary>
        [SerializeField] public TransformSyncData transSyncData;
        /// <summary>
        /// 特效实例化回调
        /// </summary>
        [NonSerialized] public Action<GameObject> EffectInstanceAction;
        #endregion
        /// <summary>
        ...
        public void OnEnable()
        {
            ...
            if (particleRandomSeed == 0)
                particleRandomSeed = (uint)URandom.Range(1, k_MaxRandInt);
        }
        ...
        public override Playable CreatePlayable(PlayableGraph graph, GameObject go)
        {
            ...
            if (prefabGameObject != null)
            {
                sourceObject = sourceGameObject.Resolve(director);
                Transform parenTransform = sourceObject != null ? sourceObject.transform : null;

                var controlPlayable = PrefabEffectPlayable.Create(graph, prefabGameObject, parenTransform, transSyncData, layer);

                sourceObject = controlPlayable.GetBehaviour().prefabInstance;
                playables.Add(controlPlayable);

                EffectInstanceAction?.Invoke(sourceObject);
            }

...
namespace Knight.Hotfix.Core.Editor
{
    ...
    {
        private const string    mGeneratePathRoot       = "../../../../KnightHotfix/Game/Generate/SerializerBinary";
        ...
        private const string    mCsprojFile             = "../../../../KnightHotfix/KnightHotfix.csproj";
        ...
        private void GenCsproj()
        {
            ...
            foreach (var file in files)
            {
                ...
                path = path.Substring(path.IndexOf("Game\\Generate\\SerializerBinary"));
                currFiles.Add(path);
            }
                ...
                if (xn.ChildNodes.Count > 0 && xn.ChildNodes[0].Name.ToLower() == "compile")
                {
                    foreach (XmlNode cxn in xn.ChildNodes)
                    {
                        if (cxn.Name.ToLower() == "compile")
                        {
                            ...
                            if (includeFile.StartsWith(@"Game\Generate\SerializerBinary\"))
                            {
